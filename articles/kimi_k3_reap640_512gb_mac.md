...
- MLX版REAPビルド: pipenetworkのKimi-K3-REAP73/REAPgraded(kimi-k3-mlx)。本記事の顕著性計測と選定はこのリポジトリのスクリプト(reap_calibrate.py / reap_plan.py)をそのまま使用。「校正コーパスに含まれないものは黙って刈り取られる」「en+code校正で中国語がtotal collapse」という実測もこのリポジトリのREADMEより。本文で「だめだった」と書いたMLXビルドの作者に、枝刈りの道具一式で助けられているという構図です。

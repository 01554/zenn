...
## どのexpertを残すかは、校正コーパスが全部決める

ここをもう少し詳しく書きます。たぶんこの記事でいちばん応用が効く部分なので。

REAPがやってることは単純で、校正テキストをモデルに流して、expertごとに「ルーターがどれくらい選んだか × 出力がどれくらい効いたか」を足し込むだけです。で、スコアの低い方から削る。つまり**校正テキストに出てこない仕事をしているexpertは、静かに消えます**。エラーも警告も出ません。消えたことに気づくのは、そのドメインの入力が来たときです。

expertがどれくらい「ドメインごとに分かれている」かは、kimi-k3-mlxに実測があります。校正をソース別に集計して、ソースごとの上位expert集合の重なりを見たもの(何もなければ偶然27%重なる設定):
...
コード系は固まっていて、欧州言語は固まっていて、CJKは固まっている。そして中国語とコードは**偶然より重ならない**。ほぼ別の脳が同居してるわけですね。だから「英語+コードで校正すると中国語がtotal collapseする」し、逆に「中国語だけで校正するとコードがtotal collapseする」(どちらもkimi-k3-mlxの実測)。私のREAP640が英語+コード特化なのは、この性質を意図的に使ったものです。

## じゃあ「日本語+マーケティング」版は作れるのか

作れるはずです。パイプラインは全く同じで、変えるのは校正コーパスだけ。
...
**(1) 配合はバイト比じゃなくトークン比で。** CJKはUTF-8で3バイト/文字なのにBPEでは密に詰まるので、バイトで混ぜると比率が狂います。kimi-k3-mlxの実測では「バイト比15%」の中国語が「トークン比32.6%」になっていました。日本語でも同じことが起きます。

**(2) 量を確保する。** 顕著性はただの統計なので、ドメインあたり数千トークンでは薄すぎます。今回は合計26万トークン流しました。狙いのドメインには最低でも数万トークンは欲しいところです。

**(3) 「マーケティング」がexpertとして分離しているかは、測るまでわからない。** 上の表で綺麗に分かれているのは「言語」と「コード」という大きな軸です。同じ日本語の中の「マーケティング文体」が専用expertを持っているかは正直怪しくて、たぶん日本語一般のexpertとかなり重なります。その場合マーケ文書を混ぜる効果は「日本語expert内の優先順位付け」程度に薄まる。本気でやるなら、先にソース別集計で重なり率を測ってから判断するのがおすすめです。

あと隠れた注意をひとつ。中国語↔日本語の42.8%という重なりは、**日本語が中国語のexpertをけっこう借りている**ことを意味します(漢字ですからね)。日本語版を作るときに中国語を完全にゼロにすると、そのぶん日本語も痩せる可能性がある。中国語も少し混ぜておくのが安全だと思います。

それと当然ですが、削った能力は本当に消えます。「日本語+マーケ」版に、あとからちょっとSQLを書かせたくなっても、もう書けません。エージェントに何をさせるかを先に全部決めて、その全部をコーパスに入れる。**校正コーパスは設定ファイルではなく仕様書**です。

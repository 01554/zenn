...
まーどうせ今年のアドベントカレンダーにはAIに書かせた記事があらゆるTECH企業から大量に出てるはずなんで、ここで一発AIで書くことが難しいScratchの話を書いて僕は記事を書く程度の事にAI使ってませんよってドヤ顔をする意味もあります。（ドヤッ）
...
![](/images/minst_predictor_on_scratch/start.png)



## 入力欄
正直に言いますが、ここが一番大変で一番時間かかりました。
MINSTは28x28の画像を入力したネットワークです

ってことは、ユーザーに28x28の画像をユーザーに書いてもらわなきゃいけません。

え、784スプライトもどうやって配置する？？
さらにスクラッチはサイズがどこにも書いてないから、わからない。

大体スプライトをどこから書くのかもわからない
ということではじめの一歩で躓いてここで投げて2日くらい塩漬けにした事は秘密です。
...
これだけです、これだけなんですが塩漬け期間含めて正味3日はかかりました。

### スプライトの作り方

![](/images/minst_predictor_on_scratch/Step1.png)
![](/images/minst_predictor_on_scratch/Step2.png)
...
スクラッチを触ってわかった事ですが、スクラッチはスプライトが全ての始まりで塊です。


なんとコードに当たる部分もスプライトに紐づけて書いていきます、僕ら的なクラスを作るとスプライトを作るがほぼイコールだと思った方がここから後がすんなりいくと思います。
...
作ったスプライトは後で拡大縮小で大きさを微調整することができます、なので実際のPixel数とかは気にしないでください。とにかく四角を書きましょう。
...
さて、この「コード」ですが、実は右下にあるスプライトと完全に紐づいています
...
っていうか、関数ブロックとかこの後で作りますが、それもグローバルとかutilとかないんで割と管理に悩む事になります。
...
![](/images/minst_predictor_on_scratch/Step4.png)

はいコードはこんなんですね、これを実行すると、とりあえず配置してくれるんですが、当然初めに作ったスプライトは大きさの調整がされてないのでめちゃくちゃな状態だったと思います。


なので、この開始座標とか、ずらす座標を調整してください。


緑の旗をクリックすれば画面を初期化してから動くんでいい感じになるまで調整します。

また、大きさについても、この時点で画面右下の「大きさ」を修正していきます。

デフォルトは100ですが、どうやらこれ固定小数点ズーム倍率(100=1.00)らしく下げるとスプライトが小さくなります。

なので、この時点でサイズを変更したり位置座標をかえることでスプライトの大きさを決定できるんですね。
...
僕は入力が反転してる事に気がついた時点で右下から左上になるよう開始座標とインクリメントの数字を変えました。

あとこの時点で気がついた人がいると思うんですが僕のコードは14x14になっています。
...
### 黒いスプライトを作成
![](/images/minst_predictor_on_scratch/Step5.png)

はい、それが終わってから反転したスプライトを用意します。
...
![](/images/minst_predictor_on_scratch/Step6.png)

さて、ここで「ドラッグできないようにする」というブロックがありますね。

でもどうやってもマウスで動かせちゃうんですよ、このスプライト達

![](/images/minst_predictor_on_scratch/step7.gif)
...
これまじで半日は悩みました、Geminiはそんなはずはないって的外れな事ばっかり俺に指示してくるのでここから先はマジAIなしで組みました。

ちなみに Geminiをクビにした理由のもう一つとして、この先からリストを扱い始めますが

Geminiに聞くと二次元配列とかlist_i で問い合わせる事ができるとか、関数定義ブロックにリストを指定できるとか、すげーうそばっかり付いてきますが僕のやる範囲では組めませんでした。
...
この「リスト」ですが、リストと変数はグローバルにできます。っていうかしないときつすぎるんで基本的に全部グローバル変数にした方がいいです。
...
![](/images/minst_predictor_on_scratch/step9.png)


この「送信を送る」がイベントリガーですね、このメッセージを受けて入力欄スプライトに以下のコードを追加して、入力欄のデータをリストに入れます。

![](/images/minst_predictor_on_scratch/step10.png)
...
## 関数作成
さて、ここまでいければ後は簡単です、普通に必要な関数を組んでいきます
...
そういえば途中のlayer構造ですが、スクラッチでは一次元配列しか使えないのが面倒だったんで

僕は 8→4→10 にしました。

![](/images/minst_predictor_on_scratch/step12.png)
